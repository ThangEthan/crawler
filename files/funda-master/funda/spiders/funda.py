...
class Funda(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        info['Postcode'] = response.xpath("//span[@class='object-header__subtitle fd-color-dark-3']//text()").get()
        info['PostcodeLetters'] = info['Postcode'][5:7] if info['Postcode'] else ""
        info['Postcode'] = info['Postcode'][0:4] if info['Postcode'] != None else ""
        info['scraped_date'] = str(datetime.datetime.now())
        info['Stad'] = response.xpath("//span[@class='object-header__subtitle fd-color-dark-3']//text()").get()
        ...
        info['VVE'] = info.pop('Bijdragevve')
        info['VVE'] = ''.join(c for c in info['VVE'] if (c.isdigit() or c == ','))
        info['Servicekosten'] = ''.join(c for c in info['Servicekosten'] if (c.isdigit() or c == ','))
